...
def test_edit(stub, tasks: Mapping[int, str]) -> Mapping[int, str]:
    edited_tasks = {}
    for task_id, _ in tasks.items():
        new_desc = random_string_generator(60)
        task = task_pb2.Task(id=task_id, description=new_desc, state=task_pb2.TaskState.ASSIGNED)
        updated_task = stub.editTask(task)
        logging.debug(f"Edited Task {pformat(updated_task)}")
        assert updated_task.description == new_desc
        edited_tasks[task_id] = new_desc
    return edited_tasks
...
if __name__ == "__main__":
    ...
    with grpc.insecure_channel("localhost:50051") as channel:
        ...
        edited_tasks = test_edit(stub, tasks)
        test_list(stub, edited_tasks)
        test_del(stub, edited_tasks)
        test_list(stub, {})
